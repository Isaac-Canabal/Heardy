...
- Playback state (queue, current media id, position, shuffle/loop, speed) is persisted to `SharedPreferences` on every `playbackEventStream` tick via `PlaybackStateService`, and restored on cold start — `main.dart` calls `MusicProvider.restorePlaybackState` from a post-frame callback (not a fixed delay), which reloads playlists/songs from SQLite, rebuilds `MediaItem`s (re-attaching `filePath`/`artPath` extras, since those aren't persisted, only IDs are), and calls either `playPlaylist` (was playing) or `restorePlaylist` (was paused, loads the source without auto-playing). `restorePlaybackState` is idempotent by real state (`AudioPlayerHandler.hasLoadedSource`) rather than a "ran once" flag, so it's safe to call more than once and self-heals if `queue`/`mediaItem` describe a song but the player has no source loaded (see "Paused-then-closed player corruption" under Known issues).
...
`LogService` appends download errors to `<app documents>/download_errors.log`; Settings has a screen to view/clear this log. `RepairService.performFullRepair` (triggered manually from Settings) is a last-resort recovery path: scans for corrupted/orphaned audio files, cleans temp files, checks DB consistency, and fully resets `AudioPlayerHandler` + `MusicProvider` state. Still kept as a general-purpose manual fallback, but as of the fix described under "Paused-then-closed player corruption" (Known issues), it should no longer be *necessary* specifically to recover from "paused, closed the app, player is broken on reopen" — that was the actual bug it was silently working around. `ErrorProvider` (`providers/error_provider.dart`) is a rolling in-memory error log but is **not currently wired into the provider tree or read by any screen** — treat it as dead/scaffolded code, not active architecture. Most services/providers also just `print()` liberally for on-device `flutter run` debugging — this is intentional throughout the codebase, not something to clean up incidentally.
...
**Pacing was tried and does NOT work — the wall is a cumulative budget, not a rate limit.** This was the obvious next lever (nothing paced successive downloads: `_maxConcurrentDownloads = 1` caps concurrency, but `processQueue`'s trailing `Future.delayed(800ms)` only separates *simultaneous* dispatches, so with one slot the loop's 300ms poll fires the next song ~300ms after the previous finishes). `tool/pacing_probe.dart` ran the same 12 videos in four blocks alternating 0s and 5s spacing, in both orders (A,B,B,A) so neither condition always got the fresh session:

| block | spacing | failures |
|---|---|---|
| 1st | none | **0%** |
| 2nd | 5s | 8% |
| 3rd | 5s | **75%** |
...
**42% failure in both conditions, identical.** What predicts failure is the *block order*, monotonically — not the spacing. So YouTube's wall is not requests-per-second; it's a **cumulative per-IP budget**, roughly 12–24 manifest requests before it comes down hard. Spacing can't help because the total is what counts, and the 150s between blocks was nowhere near enough to recover — the block accumulated across blocks instead.

This explains the reported symptom exactly: the first few songs of a playlist download fine, then everything fails and *stays* failing.

Consequences:
- `DownloadProvider._interDownloadDelay` exists and is exposed in Settings ("Descargas" → "Pausa entre canciones") but **defaults to 0**. The mechanism is kept because it's cheap and this was measured from a *desktop* IP — a mobile carrier IP (CGNAT, shared) may behave differently and is worth letting the user test. Don't raise the default without re-measuring.
- **The circuit breaker's escalation cap (180s) is too short relative to the measured recovery time.** 150s pauses did not clear the block at all. That, not pacing, is the remaining evidence-backed lever: a much longer top tier would let a big playlist complete slowly across a long window instead of burning its retries and failing everything. Not changed yet — it's a significant UX shift (downloads visibly pausing for many minutes) and needs a deliberate decision.
- Client rotation is also ruled out for this (see above: all 11 clients fail together once blocked).
