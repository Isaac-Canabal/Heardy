...
7. **In-app search** — Local/YouTube segmented control in `search_screen.dart`. Must call `DownloadSource.resolve()` on whichever result the user taps before enqueueing, same DD6 reasoning as Stage 6 — a search result's `artist` is `extract_flat`'s channel name, not the real artist.
