...
- The stream/audio download step (`_getManifestWithFallback` → `streamsClient.getManifest`) is unaffected by this fix and still uses the client-fallback cascade (`android`/`androidVr`) — it wasn't implicated in this specific failure, since the log showed the failure happening at the metadata step, not the manifest/download step.
